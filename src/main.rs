...
// --- Handle Commands (VGA + serial output) ---
fn handle_command(cmd: &[u8], arg: &[u8]) {
    ...
    if cmd_str == "help" {
        ...
        println!("  tri-compress <str> - Comprime string");
        ...
        let compressed = tri_compress::compress_str(arg_str);
        let comp_len = compressed.iter().position(|&x| x == 0).unwrap_or(64) / 2;
        let input_len = arg_str.len() as u32;
        let ratio_approx = if comp_len > 0 { (32 * 100 / comp_len as u32) as u8 } else { 100 };
        println!("Compressed '{}': {} bytes -> {} pairs (ratio ~{}%)", arg_str, input_len, comp_len, ratio_approx);  // VGA
        serial_println!("Compressed '{}': {} bytes -> {} pairs (ratio ~{}%)", arg_str, input_len, comp_len, ratio_approx);  // Serial
